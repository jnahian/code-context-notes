...
async function main() {
	...
	try {
		...
		const unitTestFiles = files.filter(f => {
			...
			return basename === 'storageManager.test.js' || basename === 'gitIntegration.test.js' || basename === 'noteDefaults.test.js' || basename === 'exportGenerator.test.js' || basename === 'exportWriter.test.js' || basename === 'agentsLink.test.js';
		});
