  ...
  implements vscode.TreeDataProvider<BaseTreeItem>
{
  ...
  setTagFilters(tags: string[], mode: "any" | "all" = "any"): void {
    ...
    const normalizedTags = [...new Set(tags.map(tag => TagManager.normalizeTag(tag)))];
    this.activeTagFilters = normalizedTags;
  ...
  private matchesTagFilter(note: Note): boolean {
    ...
    const normalizedNoteTags = note.tags.map(tag => TagManager.normalizeTag(tag));
